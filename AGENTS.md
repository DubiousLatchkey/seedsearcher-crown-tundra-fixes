...
- GPU backend is ILGPU 1.5.3 CUDA only. Never reintroduce Alea or distribute timeout-registry scripts.
...
- Device discovery uses one application-lifetime CUDA catalog; UI and execution use its same index mapping. There is no maximum GPU architecture allowlist.
...
The original 384 fixtures retain their expected values and explicitly choose CPU/CUDA through `SEEDSEARCHER_TEST_BACKEND`. Exhaustive four-IV searches are expensive, especially native CPU negative cases. Use the focused runtime tests, representative full searches and bounded native four-IV witness test for routine checks. Do not claim the full suite passed unless its complete TRX report exists. See `docs/ILGPU-validation.md` for actual results and limitations.
